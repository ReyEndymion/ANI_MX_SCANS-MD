...
return awaitconn.sendMessage(jid, buttonMessage, {
quoted,
...
await conn.sendInteractiveResponse(jid, interactiveMessage, quoted);
}
},
sendListM: {
...
async value(jid, options, quoted) {
...
}, { userJid: conn.user.jid, quoted: quoted, ephemeralExpiration: 24*60*100, disappearingMessagesInChat: 24*60*100 })
console.log('duracionefimera: ', WA_DEFAULT_EPHEMERAL)
...
groupMentions.push(text)
 console.log('mentionedGroupJid: ', [...text.matchAll(/@([0-9]{5,16}|0)/g)].map((v) => v[1] + '@g.us'))
return [...text.matchAll(/@([0-9]{5,16}|0)/g)].map((v) => v[1] + '@g.us');
},
enumerable: true,
},
getName: {
...
return conn.sendMessage(jid, { text: txt.trim(), mentions: conn.parseMention(txt) }, {quoted: quoted, ephemeralExpiration: 24*60*100, disappearingMessagesInChat: 24*60*100} );
},
enumerable: true
},
sendWritingImage: {
async value(jid, image, text, quoted) {
const isBuffer = Buffer.isBuffer(image);
const tempFilePath = path.join(temp, 'temp_image.jpg');
...
sendVideoTyping: {
async value (jid, video, text, quoted) {
const isBuffer = Buffer.isBuffer(video);
const tempFilePath = './tmp/temp_video.mp4';
