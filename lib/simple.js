...
import { info, temp, userID, groupID, lid, sBroadCastID, newsletterID } from '../config.js'
import { loadPriv, loadGroup } from './database.js';
...
const userNumbers = allTags.filter(([, , g]) => !g) .map(([match, number]) => number + lid);
return userNumbers;
},
enumerable: true
},
...
return (withoutContact ? '' : v.name) || v.subject || v.vname || v.notify || v.verifiedName || (jid.endsWith(userID) && isPnUser(jid) ? PhoneNumber('+' + jid.split('@')[0]).getNumber('international') : jid.split('@')[0]);
},
enumerable: true,
writable: true,
},
...
if (!chat || isJidBroadcast(chat)) return;
...
if (!chat || isJidBroadcast(chat)) continue;
const isGroup = isJidGroup(chat);
let chats = conn.chats[chat];
if (!chats) {
...
chat: {
get() {
const chat = this.key.remoteJid
const senderKeyDistributionMessage = this.message?.senderKeyDistributionMessage?.groupId;
return ( chat || (senderKeyDistributionMessage && senderKeyDistributionMessage !== sBroadCastID) || '').decodeJid();
},
enumerable: true,
},
typechat: {
get() {
let typeChat = ''
if (isPnUser(this.key.remoteJid) || isLidUser(this.key.remoteJid)) {
typeChat = 'privado'
} else if (isJidGroup(this.key.remoteJid)) {
typeChat = 'grupo'
} else if (isJidBroadcast(this.key.remoteJid)) {
typeChat = sBroadCastID
} else if (isJidNewsletter(this.key.remoteJid)) {
typeChat = 'canal'
} else {
typeChat = this.key.remoteJid
}
return typeChat
},
enumerable: true,
...
sender: {
get() {
if (!this.key.remoteJid) return
const sender = (isJidGroup(this.key.remoteJid)) ? this.key.fromMe ? jidNormalizedUser(this.conn.user.lid) || this.conn.user.jid : this.key.participant || this.key.participantAlt || this.key.remoteJid : this.key.fromMe ? jidNormalizedUser(this.conn.user.lid) || this.conn.user.jid : this.key.remoteJid || this.key.remoteJidAlt
...
senderJid: {
get() {
if (!this.key.remoteJidAlt || !this.key.participantAlt) isJidGroup(this.key.remoteJid) ? !this.key.participantAlt ? this.conn.resolveUser(this.key.participant) ? this.key.participantAlt = this.conn.resolveUser(this.key.participant).jid : undefined : undefined : !this.key.remoteJidAlt ? this.conn.resolveUser(this.key.remotejid) ? this.key.remoteJidAlt = this.conn.resolveUser(this.key.remotejid).jid : undefined : undefined
const senderJid = (isJidGroup(this.key.remoteJid)) ? this.key.fromMe ? this.conn.user.jid : this.key.participantAlt || this.key.remoteJidAlt : this.key.fromMe ? this.conn.user.jid : this.key.remoteJidAlt
return senderJid
},
enumerable: true,
},
...
const key = this.key
let receiver
if (this.chat === this.sender) {
if (key.fromMe) {
receiver = key.remoteJid
} else {
receiver = this.sender
}
} else if (this.chat !== this.sender) {
if (key.fromMe) {
receiver = this.chat
} else {
receiver = this.sender
...
username: {
get () {
return isJidGroup(this.key.remoteJid) ? this.key.participantUsername : this.key.remoteJidUsername
}
},
mtype: {
get() {
...
mentionedJid: {
get() {
const mentioned = this.msg?.contextInfo?.mentionedJid?.length && this.msg.contextInfo.mentionedJid.map(jid => jid) || []
return mentioned;
},
enumerable: true,
},
...
return this.conn?.reply(chatId ? chatId : this.key.remoteJid || this.key.remoteJidAlt, text, this, options);
},
},
copy: {
value() {
...
delete: {
value() {
return this.conn?.sendMessage((this.key.remoteJid || this.key.remoteJidAlt), {delete: this.key});
},
enumerable: true,
},
quoted: {
...
get() {
let typeChat = ''
if (isPnUser(self.chat) || isLidUser(self.chat)) {
typeChat = 'privado'
} else if (isJidGroup(self.chat)) {
typeChat = 'grupo'
} else if (isJidBroadcast(self.chat)) {
typeChat = sBroadCastID
} else if (isJidNewsletter(self.chat)) {
typeChat = 'canal'
} else {
typeChat = contextInfo.remoteJid
}
return typeChat
},
enumerable: true,
...
sender: {
get() {
const sender = contextInfo.participant || contextInfo.participantAlt || this.chat
return (sender || '').decodeJid();
},
},
senderJid: {
get() {
const sender = contextInfo.participantAlt//, contextInfo.participantUsername || this.chat
return (sender || '').decodeJid();//self.conn.resolveUser().jid || this.chat
},
enumerable: true,
},
fromMe: {
get() {
const sender = this.sender//, this.chatself.conn?.resolveUser()
return areJidsSameUser(sender, jidNormalizedUser(self.conn?.user.lid));
},
enumerable: true,
},
text: {
...
mentionedJid: {
get() {
const qMentioned = q.contextInfo?.mentionedJid.map(jid => jid).length > 0 ? q.contextInfo?.mentionedJid.map(jid => jid) : self.getQuotedObj()?.mentionedJid.map(jid => jid).length > 0 ? self.getQuotedObj()?.mentionedJid.map(jid => jid) : self.conn.parseMention(this.text).length > 0 ? self.conn.parseMention(this.text) : []
return qMentioned;
},
enumerable: true,
},
...
key: {
fromMe: this.fromMe,
remoteJid: this.key.remoteJid || this.key.remoteJidAlt,
id: this.id,
},
message: quoted,
...
*/
value(text, chatId, options) {
return self.conn?.reply(chatId ? chatId : this.key.remoteJid || this.key.remoteJidAlt, text, this.vM, options);
},
enumerable: true,
},
copy: {
...
value() {
const M = proto.WebMessageInfo;
return smsg(this.conn, M.create(M.toObject(this.vM)));
},
enumerable: true,
},
forward: {
...
*/
value() {
return self.conn?.sendMessage((this.key.remoteJid || this.key.remoteJidAlt), {delete: this.vM.key});
},
enumerable: true,
