...
*/
value(messageID) {
return (store.loadMessage(messageID) || Object.entries(conn.chats)
...
text = await conn.langResponse(text, userdb)
await conn.writing(jid, text)
await conn.sendMessage(jid, { video: {url: file}, caption: text.trim(), mentions: text, mimetype: 'video/mp4', caption: text }, {userJid: conn.user.jid, quoted: quoted, ephemeralExpiration: 2*60*1000 } )
fs.existsSync(tempFilePath) ? fs.unlinkSync(tempFilePath) : null
}
},
sendReact: {
