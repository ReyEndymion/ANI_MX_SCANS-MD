...
*/
value(text = '') {
return [...text.matchAll(/@([0-9]{5,16}|0)/g)].map((v) => v[1] + userID);
},
enumerable: true
},
parseGroupMention: {
...
groupJid: jidGp,
groupSubject: (options.dbGroups.data[jidGp]?.subject == undefined ? store.chats[jidGp].subject :  options.dbGroups.data[jidGp].subject) || 'Group' || await conn.getName(jidGp)//!
}
))
)
...
mentionedJid: {
get() {
const qMentioned = q.contextInfo?.mentionedJid.map(jid => this.conn.lidToJid(jid, this.chat) || jid).length > 0 ? q.contextInfo?.mentionedJid.map(jid => this.conn.lidToJid(jid, this.chat) || jid) : self.getQuotedObj()?.mentionedJid.map(jid => this.conn.lidToJid(jid, this.chat) || jid).length > 0 ? self.getQuotedObj()?.mentionedJid.map(jid => this.conn.lidToJid(jid, this.chat) || jid) : self.conn.parseMention(this.text).length > 0 ? self.conn.parseMention(this.text) : []
return qMentioned;
},
enumerable: true,
},
