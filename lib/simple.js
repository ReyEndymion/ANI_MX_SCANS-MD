...
const lid = cache.get(`pn:${user}`)
    
return { jid: id, lid: lid ? `${lid}@lid` : null, user: user, type: 'jid'}
}

if (domain === 'lid') {
const pn = cache.get(`lid:${user}`)

return { jid: pn ? `${pn}@s.whatsapp.net` : null, lid: id, user: user, type: 'lid' }
}

return { jid: id, lid: null, user: user, type: 'unknown' }
},
},
lidToJid: {
/**
...
if (sock?.chats && store?.chats && (sock.chats !== store?.chats)) {
...
chat: {
get() {
const chat = this.key?.remoteJid.endsWith('@g.us') ? this.key?.remoteJid : this.key.addressingMode === 'lid' ? this.key?.remoteJidAlt : this.key?.remoteJid
const senderKeyDistributionMessage = this.message?.senderKeyDistributionMessage?.groupId;
return ( chat || (senderKeyDistributionMessage && senderKeyDistributionMessage !== 'status@broadcast') || '').decodeJid();
},
},
isGroup: {
get() {
...
get() {
if (!this.chat) return
const sender = this.isGroup ? Object.values(this.key).find(jid => typeof jid === 'string' && (/@s\.whatsapp\.net$/.test(jid)) && !(/@lid$/.test(jid)||/status@broadcast$/.test(jid)||/:\d+@s\.whatsapp\.net$/.test(jid))) || this.key.participantAlt || this.key.participantPn || this.participant || this.key.participant : this.chat || '' 
...
chat: {
get() {
return (self.conn.resolveJid(contextInfo.participant)).jid || contextInfo.remoteJid || self.chat;
},
enumerable: true,
},
isBaileys: {
...
sender: {
get() {
const sender = self.conn.resolveJid(contextInfo.participant).jid//, this.chat
...
fromMe: {
get() {
const sender = self.conn?.resolveJid(this.sender)
...
text: {
get() {
const text = typeof q === 'string' ? self.conn.textTagsLidToJid(q, self.chat) : self.conn.textTagsLidToJid(q.text, self.chat);
