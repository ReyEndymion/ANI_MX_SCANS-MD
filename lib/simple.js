...
import { makeGroupsSocket } from './chats&groups.js';
...
//const store = (libstore.makeInMemoryStore())
let conn = (opts['legacy'] ? makeWALegacySocket : _makeWaSocket)(connectionOptions);
const newsletterMethods = makeNewsletterSocket(connectionOptions, conn);
Object.assign(conn, newsletterMethods);
Object.assign(conn, makeGroupsSocket(connectionOptions, conn))
...
const isLidGroup = groupMetadata?.owner?.endsWith(lid) || groupMetadata?.subjectOwner?.endsWith(lid) || groupMetadata.participants.some(p => p.id.endsWith(lid))
if (isLidGroup) {
...
value(text, chatId) {
try {
if (!/@(\d{5,20})/g.test(text)) return text // No hay tags

const rawIds = [...text.matchAll(/@(\d{5,20})/g)].map(v => v[1])
if (chatId.endsWith(userID)) return text // Es chat privado

if (!chatId.endsWith(groupID)) return text // No es grupo
...
if (!isLidGroup) return text // No es grupo LID

const participants = groupMetadata?.participants || []
const lidToJidMap = {}

// Crear un mapa de lid -> jid
for (const { id, jid } of participants) {
...
// Reemplazar todos los @lid que estén en el mapa
let newText = text
for (const lidNum of rawIds) {
const realJid = lidToJidMap[lidNum]
if (realJid) {
...
relayWAMessage: {
async value(jid, message, { messageId: msgId, participant, additionalAttributes, additionalNodes, useUserDevicesCache, cachedGroupMetadata, statusJidList, userdb }) {
const { logger, linkPreviewImageThumbnailWidth, generateHighQualityLinkPreview, options: axiosOptions, patchMessageBeforeSending, } = conn.ws.config;
var _a;
...
(async () => {
let groupData = cachedGroupMetadata ? await conn.cachedGroupMetadata(jid) : undefined;
...
//const jid = (0, WABinary_1.jidEncode)(isMe && isLid ? ((_b = (_a = conn.authState.creds) === null || _a === void 0 ? void 0 : _a.me) === null || _b === void 0 ? void 0 : _b.lid.split(':')[0]) || user : user, isLid ? 'lid' : 's.whatsapp.net', device);
if (isMe) {
meJids.push(jid);
}
else {
...
conn.createParticipantNodes(meJids, meMsg, mediaType ? { mediatype: mediaType } : undefined),
conn.createParticipantNodes(otherJids, message, mediaType ? { mediatype: mediaType } : undefined)
]);
...
tag: 'device-identity',
attrs: {},
content: (0, Utils_1.encodeSignedDeviceIdentity)(conn.authState.creds.account, true)
});
...
getButtonArgs: {
value(message) {
var ListType = WAProto_1.default.proto.Message.ListMessage.ListType;
...
* @param {String} text
* @param {String} footer
* @param {Object} options = {@param {Buffer} buffer}
...
imageMessage = await prepareWAMessageMedia({ image: { url: url } }, { upload: conn.waUploadToServer })
nativeFlowMessage = proto.Message.InteractiveMessage.NativeFlowMessage.fromObject({buttons: [{buttonParamsJson: JSON.stringify(buttons
...
!product ? messageContent = proto.Message.fromObject({
viewOnceMessage: {
message: {
messageContextInfo,
interactiveMessage
...
}) : messageContent = proto.Message.fromObject({
message: {
messageContextInfo,
interactiveMessage
}
...
let {title, text, buffer, buttonText, footerText, listSections, options} = messageList
title = await conn.langResponse(title, userdb)
text = await conn.langResponse(text, userdb)
buttonText = await conn.langResponse(buttonText, userdb)
footerText = await conn.langResponse(footerText, userdb)
...
body: proto.Message.InteractiveMessage.Body.fromObject({text}),
footer: proto.Message.InteractiveMessage.Footer.fromObject({footerText}),
header: title || title.header.hasMediaAttachment ? proto.Message.InteractiveMessage.Header.fromObject({
title: title,
subtitle: text,
...
let msg = await generateWAMessageFromContent(jid, messageContent, { userJid: conn.user.jid, quoted: quoted, upload: conn.waUploadToServer, ephemeralExpiration: 24*60*100, disappearingMessagesInChat: 24*60*100 })
await conn.relayWAMessage(jid, msg.message, { messageId: msg.key.id});
}
},
sendCarousel: {
/**
...
await conn.relayWAMessage(jid, msgs.message, {messageId: msgs.key.id});
} else {
...
await conn.relayWAMessage(jid, msgs.message, {
...
parseMention: {
/**
* Parses string into mentionedJid(s), con soporte lid/jid según tipo de grupo
* @param {String} text
* @return {Array<String>}
*/
value(text = '') {
...
groupJid: jidGp,
groupSubject: await conn.getName(jidGp)
}
))
)
...
console.log('sendAudioRecording: ', mime)
...
await conn.writing(jid, text);
await conn.sendPresenceUpdate('paused', jid);
await conn.sendMessage(jid, { text: text, edit: key, mentions: conn.parseMention(text) }, {quoted: quoted, ephemeralExpiration: 24*60*100, disappearingMessagesInChat: 24*60*100} );
},
enumerable: true
},
sendWritingTextCI: {
...
await conn.writing(jid, text)
await conn.sendPresenceUpdate('paused', jid);
await conn.sendMessage(jid, { image: {url: file}, caption: text.trim(), mentions: conn.parseMention(text) }, {quoted: quoted, ephemeralExpiration: 24*60*100, disappearingMessagesInChat: 24*60*100});
fs.existsSync(tempFilePath) ? fs.unlinkSync(tempFilePath) : null
},
enumerable: true
},
sendDocumentWriting: {
...
await conn.sendMessage(jid, {document: { url: file }, caption, mimetype, fileName, fileLength, pageCount, contextInfo}, { quoted, ephemeralExpiration: 24*60*100, disappearingMessagesInChat: 24*60*100});
await conn.sendPresenceUpdate('paused', jid);
return fs.existsSync(tempFilePath) ? fs.unlinkSync(tempFilePath) : null
},
enumerable: true
},
...
await conn.sendPresenceUpdate('paused', jid);
await conn.sendMessage(jid, message, {quoted: quoted, ephemeralExpiration: 24*60*100, disappearingMessagesInChat: 24*60*100});
fs.existsSync(tempFilePath) ? fs.unlinkSync(tempFilePath) : null
},
enumerable: true
},
...
let stickerBuffer;
//let tempFilePath = path.join(temp, `${Date.now()}.webp`);
...
const conv = await ffmpeg(buffer, [
'-vf', 'scale=512:512:flags=lanczos:force_original_aspect_ratio=decrease,' +
'format=rgba,pad=512:512:(ow-iw)/2:(oh-ih)/2:color=#00000000,setsar=1'
], type.ext, 'webp');
stickerBuffer = conv.data;
...
return conn.sendMessage(jid, { sticker: stickerBuffer, mimetype: 'image/webp', asSticker: true, contextInfo }, { quoted, ephemeralExpiration: 2 * 60 * 1000 });
},
enumerable: true
},
sendAudio: {
...
return conn.sendMessage(jid, {audio: { url: file }, fileName: 'error.mp3', mimetype: 'audio/mp4'}, {quoted: quoted, ephemeralExpiration: 2*60*1000})
},
enumerable: true
},
sendAudioRecording: {
...
return conn.sendMessage(jid, {audio: { url: file }, fileName: 'error.mp3', mimetype: 'audio/mpeg', ptt: true}, {quoted: quoted, ephemeralExpiration: 2*60*1000})
},
enumerable: true
},
sendVideoWriting: {
...
text = await conn.langResponse(text, userdb)
await conn.writing(jid, text)
await conn.sendMessage(jid, { video: {url: file}, caption: text.trim(), mentions: conn.parseMention(text), mimetype: 'video/mp4', caption: text }, {userJid: conn.user.jid, quoted: quoted, ephemeralExpiration: 2*60*1000 } )
fs.existsSync(tempFilePath) ? fs.unlinkSync(tempFilePath) : null
}
},
sendContact: {
...
delMessage: {
async value(jid, remoteJid, fromMe, id, user) {
return conn.sendMessage(jid, { delete: { remoteJid, fromMe, id, participant: user }})
},
enumerable: true
},
serializeM: {
...
store.bind(sock.ev);
libstore.bind(sock)
// 1. Si el socket ya traía chats legacy (por alguna razón), mézclalos al store
if (sock.chats && sock.chats !== store.chats) {
Object.assign(store.chats, sock.chats)
}

// 2. Alias: ahora todos leen/escriben el mismo objeto
sock.chats = store.chats

// 3. Repite para mensajes (por si algún plugin espera conn.messages)
if (sock.messages && sock.messages !== store.messages) {
Object.assign(store?.messages, sock.messages)
}
//sock?.messages = store.messages
return sock;
}
/**
* Serialize Message
...
sender: {
get() {
const sender = this.key?.fromMe && this.conn.lidToJid(this.conn?.user.id, this.chat) || this.conn.lidToJid(this.participant, this.chat) || this.conn.lidToJid(this.key.participant, this.chat) || this.conn.lidToJid(this.chat, this.chat) || '' 
return this.conn?.decodeJid(sender);
},
enumerable: true,
},
fromMe: {
...
mentionedJid: {
get() {
const qMentioned = q.contextInfo?.mentionedJid.map(jid => this.conn.lidToJid(jid, this.chat) || jid) || self.getQuotedObj()?.mentionedJid.map(jid => this.conn.lidToJid(jid, this.chat) || jid) || []
return qMentioned;
},
enumerable: true,
},
