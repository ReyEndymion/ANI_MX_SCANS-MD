...
*/
const { initAuthCreds, BufferJSON, proto, isJidBroadcast, isJidGroup, WAMessageStubType, useMultiFileAuthState: baileysMultiFileAuthState } = (await import('@whiskeysockets/baileys')).default;
...
if (!(jid in chats)) return chats[jid] = { id: jid }
if (!chats[jid].imgUrl) {
const url = await profilePictureUrl(jid, 'image').catch(() => join(media, 'pictures/avatar_contact.png'))
