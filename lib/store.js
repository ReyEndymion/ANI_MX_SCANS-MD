...
// @ts-ignore
const resolveJid = sock.resolveJid(chat.id?.decodeJid?.())
const id = resolveJid.jid
if (!id) continue
...
// @ts-ignore
const resolveJid = sock.resolveJid(message.key.remoteJid)
const jid = resolveJid.jid
if (!jid) continue
...
// @ts-ignore
const resolveJid = sock.resolveJid(contact.id?.decodeJid?.())
const jid = resolveJid.jid
if (!jid) continue
chats[jid] = Object.assign(chats[jid] || {}, { jid, ...contact, isContact: true })
if (typeof contact.imgUrl !== 'undefined') {
const newUrl = contact.imgUrl === null
? null
: await sock.profilePictureUrl(jid).catch(() => null);
console.log(
`contacto ${contact.jid} tiene una nueva foto de perfil: ${newUrl}`,
);
 }
}
})
...
chats[jid] = Object.assign(chats[jid] || {}, { ...chat, isChats: true })
if (isGroup && !chats[jid].metadata) await fetchGroupMetadata(jid, opts.groupMetadata)
}))
})
...
const resolveJid = sock.resolveJid(id)
const lid = resolveJid.lid
const jid = resolveJid.jid
if (!jid) continue
if (jid in chats && chat.unreadCount) chat.unreadCount += chats[jid].unreadCount || 0
chats[jid] = Object.assign(chats[jid] || {}, { id, jid, lid, ...chat, isChats: true })
}
})

sock.ev.on('presence.update', async function store(presenceUpdate) {
// @ts-ignore
const resolveJid = sock.resolveJid(presenceUpdate.id?.decodeJid?.())
const jid = resolveJid.jid
if (!jid) return
chats[jid] = Object.assign(chats[jid] || {}, presenceUpdate)
if (presenceUpdate) {
}
})
...
// @ts-ignore
const remoteJid = msg.key.remoteJid?.decodeJid?.()
const resolveJid = sock.resolveJid(remoteJid)
const jid = resolveJid.jid
const lid = resolveJid.lid
if (!jid || isJidBroadcast(jid)) continue
...
if (groupParticipantsUpdate.participants.includes(participant.phoneNumber))
participant.admin = groupParticipantsUpdate.action === 'promote' ? 'admin' : null

break
case 'remove':
metadata.participants = metadata.participants.filter(p => !groupParticipantsUpdate.participants.includes(p.phoneNumber))
break
}
})
...
// @ts-ignore
const resolveJid = sock.resolveJid(receipt.key.remoteJid)
const jid = resolveJid.jid
const lid = resolveJid.lid
if (!jid) continue
