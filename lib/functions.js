...
const resetName = path.basename(pathSession).replace('Session', '')
...
process.send(`reset ${resetName}`)
}
} else process.send(`reset ${resetName}`)
...
export async function dataSubBot(name, dbRegisterSubBot) {
await dbRegisterSubBot.read()
...
return fs.unlinkSync(file) // 3 minutes
return false 
})
}
...
conn.logger.warn(`(${botName}) Archivo o carpeta eliminada - '${filename}'`);
}
}
} catch (e) {
...
const botName = path.basename(pluginFolder).replace('plugins', '');
...
reload('add', filename, conn, pluginFolder, botName) }).on('change', (file) => { 
const filename = path.relative(pluginFolder, file) 
reload('change', filename, conn, pluginFolder, botName) }).on('unlink', (file) => { 
const filename = path.relative(pluginFolder, file) 
reload('unlink', filename, conn, pluginFolder, botName) }).on('addDir', (dir) => { 
const dirname = path.relative(pluginFolder, dir) 
reload('addDir', dirname, conn, pluginFolder, botName) }).on('unlinkDir', (dir) => { 
const dirname = path.relative(pluginFolder, dir) 
...
if (users.id === lid) return users.jid;

if (users.jid === lid) return users.jid;
}
...
export async function ajusteTiempo(ms) {
...
const { modes, operators } = await import("./constants.js");
...
return localHash === newHash // true = iguales, false = distintos
}
...
try {
options ? options : {}
const res = await axios({ method: 'GET', url: url, headers: {...DEFAULT_HEADERS}, ...options })
return res.data
} catch (err) {
return err
}
