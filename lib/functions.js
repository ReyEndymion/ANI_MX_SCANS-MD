...
export function mergeDefined(target, source) {
    ...
    for (const [key, value] of Object.entries(source)) {
        ...
        // Si es un objeto, conservar y completar sus propiedades
        if (
            typeof value === 'object' &&
            !Array.isArray(value) &&
...
export function reNewSession(folderPath) {
...
return `*[❗] Se eliminaron ${filesDeleted} archivos de sesión, excepto el archivo creds.json.*`
}
...
export async function purgeKeySession(folderPath, key) {
const chatId = key.remoteJid.endsWith(groupID) ? [key.remoteJid, key?.participant, key?.participantAlt, key?.participantPn] : [key.remoteJid];
const files = fs.readdirSync(folderPath);
let filesDeleted = 0;
for (const file of files) {
...
export async function maker(url, text) {
