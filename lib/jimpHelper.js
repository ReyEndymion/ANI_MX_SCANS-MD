...
const target = Jimp.cssColorToHex(color);
...
async function tempFile(ext = '.bin') {
const {temp} = await import('../config.js')
return path.join(temp, `tmp-${Date.now()}-${Math.random().toString(36).slice(2)}${ext}`)
}
...
samples.push(Jimp.intToRGBA(image.getPixelColor(x, 0)));
samples.push(Jimp.intToRGBA(image.getPixelColor(x, h - 1)));
}
for (let y = 0; y < h; y += step) {
samples.push(Jimp.intToRGBA(image.getPixelColor(0, y)));
samples.push(Jimp.intToRGBA(image.getPixelColor(w - 1, y)));
}
...
const charWidth = Jimp.measureText(font, "Ｍ");
const lineHeight = Jimp.measureTextHeight(font, "A", 1000);

const maxLineWidth = Math.max(...lines.map(line => Jimp.measureText(font, line)));
...
image.print(font, 10, y, line);
y += (lineHeight || Jimp.measureTextHeight(font, line, maxLineWidth));
}
...
image.print(font, offsetX + j * cellSize, 0, { text: String(j), alignmentX: Jimp.HORIZONTAL_ALIGN_CENTER, alignmentY: Jimp.VERTICAL_ALIGN_MIDDLE }, cellSize, cellSize)
}
for (let i = 0; i < LADO; i++) {
image.print(font, 0, offsetY + i * cellSize, { text: String(i), alignmentX: Jimp.HORIZONTAL_ALIGN_CENTER, alignmentY: Jimp.VERTICAL_ALIGN_MIDDLE }, cellSize, cellSize)
for (let j = 0; j < LADO; j++) {
image.print(font, offsetX + j * cellSize, offsetY + i * cellSize, { text: array[i][j] || '', alignmentX: Jimp.HORIZONTAL_ALIGN_CENTER, alignmentY: Jimp.VERTICAL_ALIGN_MIDDLE }, cellSize, cellSize)
