...
if (!effect) throw '*[❗INFO❗] ¿COMO USAR ESTE COMANDO?*\n—◉ _#logo (efecto) (texto)_\n*EJEMPLO:*\n—◉ _#logo 3d-deep-sea-metal Mystic_\n\n*[❗] CUANDO LES DIGA QUE HACE FALTA UN TEXTO EL USO SERIA:*\n—◉ _#logo (efecto) (texto1|texto2)_\n*EJEMPLO:*\n—◉ _#logo Wolf-Logo-Galaxy Mystic|Bot_\n\n*<_LISTA DE EFECTOS_/>*\n\n° ඬ⃟📝 #logo ' + effects.map(v => v.title).join('\n° ඬ⃟📝 #logo ')
effect = effect.toLowerCase()
if (!effects.find(v => (new RegExp(v.title, 'gi')).test(effect))) throw `*[❗INFO❗] EL EFECTO ${effect} NO ESTA EN LA LISTA DE EFECTOS*`
let text = txt.replace(new RegExp(effect, 'gi'), '').trimStart()
if (text.includes(split)) text = text.split(split)
text = Array.isArray(text) ? text : [text]
let res = await textpro(effect, ...text)
if (typeof res == 'number') throw res == -1 ? `*[❗INFO❗] EL EFECTO ${effect} NO ESTA EN LA LISTA DE EFECTOS*` : `*[❗INFO❗] EL USO CORRECTO DEL COMANDO ES ${usedPrefix + command} ${effect} ${new Array(res).fill('texto').map((v, i) => v + (i ? i + 1 : '')).join('|')}*`
