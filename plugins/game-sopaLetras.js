...
let { default: request } = await import('request');
...
// return conn.sendWritingText(m.chat, ¡Hola! ¿Te gustaría jugar a una sopa de letras? Solo di 'Sí' para empezar., userdb, m); 
...
conn.sendWritingText(m.chat, `no se pudo generar lista, por favor reintente mas tarde`, m)
}
})
}
...
image.write(path.join(temp, 'sopa-de-letras.jpg'));
...
conn.sendWritingText(m.chat, `Encuentra estas palabras:wordsList`, userdb, m);
...
return conn.sendWritingText(m.chat, `¡Se acabó el tiempo! ¿Quieres volver a jugar?`, userdb, m);
}, 30000); // 30 segundos
})
}
handler.help = ['juegos']
