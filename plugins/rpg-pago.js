...
let handler = async (m, {conn, start, info, isPrems, userdb, db, senderJid}) => {
let fkontak = { "key": { "participants":"0@s.whatsapp.net", "remoteJid": "status@broadcast", "fromMe": false, "id": "Halo" }, "message": { "contactMessage": { "vcard": `BEGIN:VCARD\nVERSION:3.0\nN:Sy;Bot;;;\nFN:y\nitem1.TEL;waid=${senderJid.split('@')[0]}:${senderJid.split('@')[0]}\nitem1.X-ABLabel:Ponsel\nEND:VCARD` }}, "participant": "0@s.whatsapp.net" 
}
let users = senderJid.split`@`[0]
...
if (new Date - userdb.lastpago < 432000000) return await conn.sendButton(m.chat, `💰 SU SIGUIENTE PAGO ES EN ...`, info.nanipe + `\n\nVUELVE EN : COME BACK IN\n${clockString(time - new Date() * 1)}`, null, [['M E N U ☘️', '/menu']], fkontak, m)
let texto = ''
...
return conn.sendWritingText(m.chat, resp+buff, fkontak)
}
...
return conn.sendWritingText(m.chat, `USTED NO TIENE AUTORIZACIÓN PARA USAR ESTE COMANDO.`, userdb, m)}
}
handler.command = ['pgg'] 
handler.level = 10
handler.rowner = true
