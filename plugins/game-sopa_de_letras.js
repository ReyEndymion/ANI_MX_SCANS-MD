...
if (!userSP) {
userSP = senderJid.split("@")[0]
await conn.sendWritingText(m.chat, `*@${senderJid.split("@")[0]} REGISTRADO EN EL JUEGO* ✅`, userdb, m)
}
...
await conn.sendWritingText(m.chat, `🔠 *SOPA DE LETRAS* 🔠
*PALABRA:* \`\`\`"${PALABRA}"\`\`\`
...
➡️ \`\`\`FILA 2\`\`\`⬇️ \`\`\`COLUMNA 8\`\`\``.trim(), userdb, m)
await conn.sendWritingText(m.chat, `🔠 *${PALABRA.split("").join(" ")}* 🔠\n\n` + sopaDeLetrasConBordes.trimEnd(), userdb, m)
fila = filaInicial 
...
if (userSP != senderJid.split("@")[0]) {
await conn.sendWritingText(m.chat, `*@${tagUser.split("@")[0]} ESTA JUGANDO SOPA DE LETRAS 🔠 ACTUALEMENTE*`, userdb, m)
return
}
if (intentos === 0) {
intentos = 3 
...
if (intentos !== 0) {
await conn.sendWritingText(m.chat, `*@${senderJid.split("@")[0]} TE QUEDA UN MINUTO!!* 😨`, userdb, m)
}
await new Promise((resolve) => setTimeout(resolve, 3 * 60 * 1000)) // 3 min
if (intentos !== 0) {
await conn.sendWritingText( m.chat, `*@${senderJid.split("@")[0]} EL TIEMPO SE HA ACABADO!!* 😧\n\n*LA PALABRA _"${sopaPalabra}"_ SE ENCONTRABA EN LA DIRECCIÓN _${cambioLetra}_ DE LA FILA _${fila}_ Y COLUMNA _${columna}_*`, userdb, m)
...
usersdb[senderJid].limit += diamante

await conn.sendWritingText(m.chat, `\`\`\`🎊 HAS GANADO ${diamante} ${rpgshop.emoticon('limit')}!!\`\`\`\n\n*CORRECTO!! LA PALABRA _"${sopaPalabra}"_ SE ENCONTRABA EN LA DIRECCIÓN _${cambioLetra}_ DE LA FILA _${fila}_ Y COLUMNA _${columna}_*`, userdb, m)
fila = null, columna = null, sopaNube = null, sopaPalabra = null, sopaDir = null, userSP = null, cambioLetra = null
intentos = 0
} else {
if (intentos === 1) {
fila = null, columna = null, sopaNube = null, sopaPalabra = null, sopaDir = null, userSP = null, cambioLetra = null
intentos = 0
await conn.sendWritingText(m.chat, `🫡 *AGOTASTE LOS INTENTOS!! LA PALABRA _"sopaPalabra"_ SE ENCONTRABA EN LA DIRECCIÓN _${cambioLetra}_ DE LA FILA _${fila}_ Y COLUMNA _${columna}_*`, userdb, m)
return 
} else {
intentos -= 1
await conn.sendWritingText(m.chat, `😮‍💨 *INCORRECTO. TE QUEDAN _${intentos}_ INTENTOS!!*${intentos === 1 ? '' : `\n*PALABRA A ENCONTRAR:* \`\`\`${sopaPalabra}\`\`\``}\n\n${intentos === 1 ? `\`\`\`💡 PISTA!!\`\`\`\n*LA PALABRA _${sopaPalabra}_ SE ENCUENTRA EN LA DIRECCIÓN _"${cambioLetra}"_*\n\n` : ''}${sopaNube}`, userdb, m)
}}
}
}
