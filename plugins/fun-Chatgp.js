...
let time = userdb.msgwait + 300000
if (new Date - db.data.users[senderJid].msgwait < 300000) return conn.sendWritingText(m.chat, `*_⚠️ • Tienes que esperar ${msToTime(time - new Date())} para volver a enviar un mensaje._*`, userdb, m)
...
let fakegif = { key: {participant: `0@s.whatsapp.net`, ...("5215532867844-1600616542@g.us" ? { remoteJid: "5215532867844-1600616542@g.us" } : {})},message: {videoMessage: { title: info.nanie, h: `Hmm`,seconds: '99999', gifPlayback: 'true', 'caption': info.nanie, jpegThumbnail: false }}}
let teks = `*🌺 • Grupo:* ${groupMetadata.subject}\n*🍀 • De:* ${name}\n*🍁 • Nú𝚖ero:* wa.me/${who.split`@`[0]}\n*📧 • Mensaje:* ${text}`
for (let id of groups) {

await conn.sendWritingText(id, teks, userdb, fakegif)
userdb.msgwait = new Date * 1
}}
